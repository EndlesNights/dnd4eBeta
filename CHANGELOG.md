...
* __Sync Senses to Token Vision:__ This new system setting is on by default, allowing a token's vision to be inferred automatically based on the token's actor's senses. For fine0grained control, token config takes predence over inferred settings. (SagaTympana)

...
// import { dnd4eAltusPrepopulate } from '../setup/dnd4eAltus-prepopulate.js';
...
export class DnD4eActorSheetV2 extends ActorSheet4e {
  ...
  static get defaultOptions() {
    ...
    mergeObject(options, {
      classes: options.classes.concat(['dnd4eAltus-v2', 'actor', 'character-sheet']).filter(c => c !== 'dnd4eAltus'),
      width: 960,
      height: 960,
  ...
  /** @override */
  get template() {
    return `systems/dnd4eAltus/templates/actors/actor-character-sheet-v2.html`;
  }
  ...
  render(force=false, options={}) {
    ...
    .then(rendered => {
      ...
      let el = this.element.find('.dnd4eAltus-vueport');
  ...
  async _createItem(event) {
    ...
    // Default image.
    let img = CONFIG.dnd4eAltus.defaultTokens[itemType] ?? CONFIG.DEFAULT_TOKEN;
    ...
    let item = await this.actor.createOwnedItem({
      name: 'New ' + game.i18n.localize(`dnd4eAltus.${itemType}`),
      type: itemType,
      img: img,
      data: data
    });
  ...
  async _onSaveRoll(difficulty) {
    ...
    let label = game.i18n.localize(`dnd4eAltus.SAVE.${difficulty}`);
    ...
    const template = `systems/dnd4eAltus/templates/chat/save-card.html`
  ...
  async _onIconRoll(iconIndex) {
    ...
    if (actorData.icons[iconIndex]) {
      ...
      // Basic template rendering data
      const template = `systems/dnd4eAltus/templates/chat/icon-relationship-card.html`
  ...
  async _onCommandRoll(dice) {
    ...
    // Basic template rendering data
    const template = `systems/dnd4eAltus/templates/chat/command-card.html`
    const token = actor.token;
   ...
   */
   _onRest(event) {
    ...
    // Otherwise, we need to make a dialog.
      ...
      let options = {
        title: null,
        confirmLabel: 'dnd4eAltus.CHAT.Rest',
        cancelLabel: 'dnd4eAltus.CHAT.Cancel',
        default: 'rest',
      };

      if (type == 'quick') {
        options.title = 'dnd4eAltus.CHAT.QuickRest';
        options.content = 'dnd4eAltus.CHAT.QuickRestBody';
      }
      else if (type == 'full') {
        options.title = 'dnd4eAltus.CHAT.FullHeal';
        options.content = 'dnd4eAltus.CHAT.FullHealBody';
      }
  ...
  // async _importPowers(event) {
    ...
    // let prepop = new dnd4eAltusPrepopulate();

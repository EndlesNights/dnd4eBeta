export class Helper {
   ...
   */
	...
	static _preparePowerCardData(chatData, CONFIG, actorData=null) {
		...
		if(chatData.trigger) {
			powerDetail += `<p class="trigger"><strong>${game.i18n.localize("DND4EBETA.Trigger")}:</strong> ${chatData.trigger}</p>`;
		}
		...
		if(chatData.target && (typeof chatData.target === "string")) { //target can sometimes be an object for things that did not have a dropdown
			powerDetail += `<p class="target"><strong>${game.i18n.localize("DND4EBETA.Target")}:</strong> ${chatData.target}</p>`;
		}

		if(!chatData.postEffect && chatData.effect.detail) {
			powerDetail += `<p class="effect alt"><strong>${game.i18n.localize("DND4EBETA.Effect")}:</strong> ${chatData.effect.detail}</p>`;
		}
		
		if(!chatData.postSpecial && chatData.special) {
			powerDetail += `<p class="special"><strong>${game.i18n.localize("DND4EBETA.Special")}:</strong> ${chatData.special}</p>`;
		...
		if(chatData.attack.isAttack) {
			if(chatData.attack.ability === "form"){
				...
				powerDetail += `<p class="attack"><strong>${game.i18n.localize("DND4EBETA.Attack")}:</strong> ${trimmedForm} ${game.i18n.localize("DND4EBETA.VS")} ${CONFIG.DND4EBETA.def[chatData.attack.def]}</p>`;
			}
			else if(chatData.attack.ability){
				powerDetail += `<p class="attack"><strong>${game.i18n.localize("DND4EBETA.Attack")}</strong>: ${CONFIG.DND4EBETA.abilities[chatData.attack.ability]} ${game.i18n.localize("DND4EBETA.VS")} ${CONFIG.DND4EBETA.def[chatData.attack.def]}</p>`;
			} else {
				powerDetail += `<p class="attack"><strong>${game.i18n.localize("DND4EBETA.Attack")}</strong>: ${game.i18n.localize("DND4EBETA.Attack")} ${game.i18n.localize("DND4EBETA.VS")} ${CONFIG.DND4EBETA.def[chatData.attack.def]}</p>`;
			}
			// powerDetail += `<p class="attack"><strong>${game.i18n.localize("DND4EBETA.Attack")}</strong>: ${CONFIG.DND4EBETA.abilities[chatData.attack.ability] || "Attack"} ${game.i18n.localize("DND4EBETA.VS")} ${CONFIG.DND4EBETA.def[chatData.attack.def]}</p>`;
		}

		let highlight = true;
		if (chatData.hit.detail){
			powerDetail += `<p class="hit ${highlight? `alt`: ``}"><strong>${game.i18n.localize("DND4EBETA.Hit")}:</strong> ${chatData.hit.detail}</p>`;
			highlight = !highlight;
		}

		if (chatData.miss.detail){
			powerDetail += `<p class="miss ${highlight? `alt`: ``}"><strong>${game.i18n.localize("DND4EBETA.Miss")}:</strong> ${chatData.miss.detail}</p>`;
			highlight = !highlight;
		}

		if(chatData.postEffect && chatData.effect.detail) {
			powerDetail += `<p class="effect ${highlight? `alt`: ``}"><strong>${game.i18n.localize("DND4EBETA.Effect")}:</strong> ${chatData.effect.detail}</p>`;
			highlight = !highlight;
		}
		if(chatData.postSpecial && chatData.special) {
			powerDetail += `<p class="special ${highlight? `alt`: ``}"><strong>${game.i18n.localize("DND4EBETA.Special")}:</strong> ${chatData.special}</p>`;
			highlight = !highlight;
			for (let [i, entry] of Object.entries(chatData.specialAdd.parts)){
		...
		if(chatData.sustain?.actionType !== "none" && chatData.sustain?.actionType) {
			powerDetail += `<p class="sustain ${highlight? `alt`: ``}"><strong>${game.i18n.localize("DND4EBETA.Sustain")} ${CONFIG.DND4EBETA.abilityActivationTypes[chatData.sustain.actionType]}:</strong> ${chatData.sustain.detail}</p>`;
		}

...
export default class ActiveEffectConfig4e extends ActiveEffectConfig {
	...
	*/
	async _addEffectDot() {
		const i = this.document.flags.dnd4eAltus.dots.length;
		//const idx = this.document.flags.dnd4eAltus.dots.length;
		return this.submit({preventClose: true, updateData: {
			[`flags.dnd4eAltus.dots.${i}`] : {amount: "", type: "", typesArray: []}
		}});
	}
	...
	*/
	_getSubmitData(updateData={}) {
		...
		data.flags.dnd4eAltus.dots = Array.from(Object.values(data.flags.dnd4eAltus.dots || {}));
		if (data.flags.dnd4eAltus.dots.length){
			for (let [i, dot] of data.flags.dnd4eAltus.dots.entries()){
				data.flags.dnd4eAltus.dots[i].amount = dot.amount;
				data.flags.dnd4eAltus.dots[i].typesArray = dot.typesArray.sort();
			}
		}

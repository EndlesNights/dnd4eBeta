...
export default class AttributeBonusDialog extends DocumentSheet4e {
	...
	static #onBonusAdd(event, target) {
		...
		const bonusData = utils.byString(this.options.target, this.document).bonus;
		const newBonus = [{}];
	...
	static #onBonusDelete(event, target) {
		...
		const bonus = foundry.utils.duplicate(utils.byString(this.options.target, this.document).bonus);

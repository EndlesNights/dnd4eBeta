...
export default class CustomSkillConfig extends FormApplication {
	static get defaultOptions(){
		return mergeObject(super.defaultOptions, {
			...
			template: "systems/dnd4eAltus/templates/apps/custom-skill-config.html",
			width: 835,
			height: 'auto',
			closeOnSubmit: true,
	...
	async getData(options) {
		const customSkills = game.settings.get("dnd4eAltus", "custom-skills");
		return {customSkills};
	}
	...
	async _updateObject(event, formData) {
		...
		if(!Object.keys(formData).length) {
			...
			return game.settings.set("dnd4eAltus", "custom-skills", {});
		}

		const coreData = game.dnd4eAltus.config.coreSkills

		if(!Array.isArray(formData[Object.keys(formData)[0]])){

			formData = [formData].filter(newObj => {
				if (coreData[newObj.id]) {
					console.log(`Aborted Custom Skill creation of <strong>"${newObj.label}"</strong>, for matching id of <strong>"${newObj.id}"</strong> with core skill <strong>"${game.dnd4eAltus.config.skills[newObj.id]}"</strong>.`);
					ui.notifications.warn(`Aborted Custom Skill creation of <strong>"${newObj.label}"</strong>, for matching id of <strong>"${newObj.id}"</strong> with core skill <strong>"${game.dnd4eAltus.config.skills[newObj.id]}"</strong>.`);
					return false;
				}
				return true;
			});
			...
			return game.settings.set("dnd4eAltus", "custom-skills", formData);
		}
		// let newSKills
		...
		return game.settings.set("dnd4eAltus", "custom-skills", transposedArray);
	}

	updateActorsSkills(newDataArray){
		const oldData = game.settings.get("dnd4eAltus", "custom-skills");
		...
		return game.settings.set("dnd4eAltus", "custom-skills", newDataArray);
	}

	async addCustomSkill(skillData){

		game.dnd4eAltus.config.skills[skillData.id] = skillData.label;
	...
	async removeCustomSkill(skillData){
		...
		delete game.dnd4eAltus.config.skills[skillData.id];
		for(const actor of game.actors){
	...
	activateListeners(html) {
		...
		$(this.element).on("click", "[custom-skill-create]", (ev) => {
			...
			renderTemplate("systems/dnd4eAltus/templates/apps/custom-skill-partial.html", {
				id: ID,
				onResult: [],
